...
class MyGameSolver(GameDleSolver):
    def __init__(self):
        super().__init__("Narutodle.csv", "Character")
    
    def _define_category_types(self):
        # Define which categories can have partial matches
        self.partial_matchable = ["Affiliation", "Jutsu Types", "Kekkei Genkai", "Release Types", "Attributes"]
        
        # Define which categories are exact match only
        self.yes_or_no = ["Gender"]
        
        # Define which categories can be compared (before/after/correct)
        self.orderable = ["Debut Arc"]
    
    def get_category_config(self) -> List[Tuple[str, List[str]]]:
        return [
            ('gender', ['correct', 'incorrect']),
            ('affiliation', ['correct', 'partial', 'incorrect']),
            ('jutsu types', ['correct', 'partial', 'incorrect']),
            ('kekkei genkai', ['correct', 'partial', 'incorrect']),
            ('release types', ['correct', 'partial', 'incorrect']),
            ('attributes', ['correct', 'partial', 'incorrect']),
    ...
    def _extract_arc_number(self, arc_string: str) -> int:
        """Extract the numeric prefix from arc string (e.g., '01. Prologue — Land of Waves' -> 1)"""
        ...
        for category in self.orderable:
            ...
            if feedback_key in feedback:
                ...
                if category == "Debut Arc":
                    ...
                    guessed_arc_num = self._extract_arc_number(guessed_value)
